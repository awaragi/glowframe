...
- **THEN** the encoded parameter contains the mode discriminant, all mode-specific fields, and the profile's `clock` fields, but no `id` field

### Requirement: Decode and validate incoming profile URL parameter
On app load, the system SHALL detect a `?profile=` query parameter, decode it with `decodeURIComponent` + `JSON.parse`, and validate the result against a Zod discriminated-union schema covering all 6 profile modes and the shared `clock` configuration.
...
The system SHALL present a Radix UI Dialog asking the user to confirm or dismiss the incoming profile import. The dialog SHALL display the incoming profile's name and mode. When confirmed, the imported preset SHALL retain the shared clock configuration, be added to the store with a new generated `id`, and become the active preset.

#### Scenario: User confirms import
- **WHEN** the user clicks "Import" in the confirmation dialog
- **THEN** the profile is added to the store with a new generated `id`, its shared `clock` configuration is preserved, it is set as the active profile, the `?profile=` parameter is removed from the URL via `history.replaceState`, and a success toast is shown
