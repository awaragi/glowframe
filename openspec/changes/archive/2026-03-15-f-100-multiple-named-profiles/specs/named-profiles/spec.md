...
The system SHALL define a `Profile` type with fields: `id` (UUID string), `name` (string, max 64 characters), `lightColor` (hex string), and `brightness` (integer 0–100).

#### Scenario: Profile has all required fields
- **WHEN** a profile object is created
- **THEN** it contains `id`, `name`, `lightColor`, and `brightness` with the correct types
...
The system SHALL seed a single profile named "Default" with `lightColor: "#ffffff"` and `brightness: 100` when no prior localStorage state exists.

#### Scenario: First launch seeds default profile
- **WHEN** the app loads with no prior localStorage state
- **THEN** `profiles` contains exactly one entry named "Default" and `activeProfileId` matches its `id`
