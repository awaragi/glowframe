...
**Reason:** Replaced by per-mode rendering requirements. There is no longer a single flat `lightColor`, `brightness`, or `colorTemperature` on all profiles; each mode defines its own color/brightness fields.
**Migration:** See ADDED Requirements — per-mode renderer requirements below define how each mode computes its displayed colors.

### Requirement: Ring format controls light shape
**Reason:** `ringFormat` is removed; light shape is now determined entirely by the profile's `mode` discriminant.
**Migration:** `mode: 'ring'` / `mode: 'ring-color'` replace `ringFormat: 'circle'`. `mode: 'full'` / `mode: 'full-color'` replace `ringFormat: 'full'`. `ringFormat: 'border'` has no replacement.
