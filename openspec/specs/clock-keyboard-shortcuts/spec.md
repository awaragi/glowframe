...
Keyboard shortcuts for controlling the clock overlay without opening the settings modal. Provides a single `T` key that cycles through clock visibility and position states, and `Shift+T` to cycle clock size.
