...
### F-250 — Profile Schema Refactor (`light` + `clock` sub-objects)
...
Profile {
  ...
  light: LightSettings  // All current light-surface settings (previously flat on Profile)
  clock: ClockSettings  // All clock settings (previously flat on Profile, introduced in F-220)
}
```

- **`LightSettings`** contains every field currently defined directly on the profile in F-110:
...
- Define `LightSettingsSchema` and `ClockSettingsSchema` as separate named Zod objects.
- Compose them into `ProfileSchema` using `z.object({ id, name, light: LightSettingsSchema, clock: ClockSettingsSchema })`.
- All existing Zod refinements (e.g., `innerRadius < outerRadius` from F-160) are preserved inside `LightSettingsSchema`.

#### Zustand Store

- All store actions that currently read or write flat profile fields must be updated to target the correct sub-object (e.g., `updateLight(partial: Partial<LightSettings>)`, `updateClock(partial: Partial<ClockSettings>)`).
...
- Every component, hook, and utility that accesses profile fields must be updated to use the new paths (e.g., `profile.light.brightness` instead of `profile.brightness`).
...
- Unit tests must cover:
  - `LightSettingsSchema` and `ClockSettingsSchema` validate correct inputs and reject invalid ones.
  - The Zustand migration function correctly converts a flat v1 profile record to the new nested v2 shape.
  - `updateLight` and `updateClock` actions write only to their respective sub-objects without affecting the other.
